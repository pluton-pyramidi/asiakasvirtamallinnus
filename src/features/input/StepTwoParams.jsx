...
export default function StepTwoParams() {
  ...
  const insufficiencyRateStepTwo = useSelector(calculateInsufficencyRateStepTwo);
  ...
  return (
    <Box>
      <PercentageInput
        ...
        handleChange={(e) => handleInputChange("sufficiencyRateStepTwo", Number(e.target.value))}
      />
      <Typography>Hoito ei riitä (jatkohoidon tarve): {Math.round(insufficiencyRateStepTwo * 100)} %</Typography>
      <PercentageInput
        label="Jatkohoito: Ohjataan muuhun hoitoon per kuukausi"
        value={input.stepTwoToMuuRate}
        handleChange={(e) => handleInputChange("stepTwoToMuuRate", Number(e.target.value))}
      />
      <Typography>
        Jatkohoito: Asiakas palaa suoraan takaisin jonoon per kuukausi: {Math.round(stepTwoToQueueRate * 100)} %
      </Typography>
      <NumberInput
        label="Tapaamisen keskimääräinen kesto sis. valmistautumisen ja kirjaukset (min)"
        value={input.meetingDurationStepTwo}
        handleChange={(e) => handleInputChange("meetingDurationStepTwo", Number(e.target.value))}
      />
      <PercentageInput
